...
class TrainingSampler(Sampler):
    ...
    def __iter__(self):
        ...
        # itertools.isclice: iterable에서 선택된 요소들 반환
        # rank부터 시작해서 world_size만큼씩 건너뛰면서 반환됨 -> 모든 worker에게 균등 분배 되도록

    def _infinite_indices(self):
        g = torch.Generator() # generates pseudo random numbers
        g.manual_seed(self._seed)
        while True:
            if self._shuffle:
                yield from torch.randperm(self._size, generator=g).tolist() # 값을 하나씩 밖으로 내보냄
            else:
