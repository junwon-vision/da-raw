...
def fast_rcnn_inference_single_image(
    ...
    scores = scores[:, :-1] # get rid of background score
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def losses(self, predictions, proposals, half = False):
        """
        Args:
        ...
        if not half:
            ...
            mid_dim = int(len(scores)/2)
            if self.use_sigmoid_ce:
                loss_cls = self.sigmoid_cross_entropy_loss(scores[:mid_dim], gt_classes[:mid_dim])
            else:
                loss_cls = cross_entropy(scores[:mid_dim], gt_classes[:mid_dim], reduction="mean")

            losses = {
                "loss_cls": loss_cls,
                "loss_box_reg": self.box_reg_loss(
                    proposal_boxes[:mid_dim], gt_boxes[:mid_dim], proposal_deltas[:mid_dim], gt_classes[:mid_dim]
                ),
            }
    ...
    def inference(self, predictions: Tuple[torch.Tensor, torch.Tensor], proposals: List[Instances]):
        ...
        scores = self.predict_probs(predictions, proposals) # softmax output of predictions
